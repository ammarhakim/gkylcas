...
(provide train-lax-friedrichs-vector3-2d
         train-lax-friedrichs-vector3-2d-second-order
         train-roe-vector3-2d
         train-roe-vector3-2d-second-order)

;; ----------------------------------------------------------------------------------------------------
;; Train a Lax–Friedrichs (Finite-Difference) Surrogate Solver for a 2D Coupled Vector System of 3 PDEs
;; ----------------------------------------------------------------------------------------------------
(define (train-lax-friedrichs-vector3-2d pde-system neural-net
                                         #:nx [nx 200]
                                         #:ny [ny 200]
                                         #:x0 [x0 0.0]
                                         #:x1 [x1 2.0]
                                         ...
                                         #:init-funcs [init-funcs (list
                                                                   ...
                                                                   `0.0
                                                                   `0.0)])
  "Generate C code that trains a surrogate solver for the 2D coupled vector system of 3 PDEs specified by `pde-system` using the Lax-Friedrichs finite-difference method,
...
// Train a Lax–Friedrichs first-order finite-difference surrogate solver for a coupled vector system of 3 PDEs in 2D.
      ...
      for (int i = 1; i <= nx; i++) {
        for (int j = 1; j <= ny; j++) {
          ...
          for (int k = 0; k < 3; k++) {
            ...
            output_data[k][(n * nx * ny) + ((i - 1) * ny) + (j - 1)][0] = u[i][(j * 3) + k];
          }
        }
      }
    }
...
;; -------------------------------------------------------------------------------------------------------------------------------------------
;; Train a Lax–Friedrichs (Finite-Difference) Surrogate Solver for a 2D Coupled Vector System of 3 PDEs with a Second-Order Flux Extrapolation
;; -------------------------------------------------------------------------------------------------------------------------------------------
(define (train-lax-friedrichs-vector3-2d-second-order pde-system limiter neural-net
                                                      #:nx [nx 200]
                                                      #:ny [ny 200]
                                                      #:x0 [x0 0.0]
                                                      #:x1 [x1 2.0]
...
// AUTO-GENERATED CODE FOR TRAINING ON COUPLED VECTOR PDE SYSTEM: ~a
// FLUX LIMITER: ~a
// Train a Lax–Friedrichs first-order finite-difference surrogate solver for a coupled vector system of 3 PDEs in 2D, with a second-order flux extrapolation.
  ...
  while (t < t_final) {
    ...
    for (int i = 1; i <= nx + 2; i++) {
      for (int j = 1; j <= ny + 2; j++) {
        for (int k = 0; k < 3; k++) {
          double r = (u[i][(j * 3) + k] - u[i][((j - 1) * 3) + k]) / (u[i][((j + 1) * 3) + k] - u[i][(j * 3) + k]);
    ...
    for (int i = 2; i <= nx + 1; i++) {
      for (int j = 2; j <= ny + 1; j++) {
        ...
        // Update the conserved variable vector.
        for (int k = 0; k < 3; k++) {
          un[i][(j * 3) + k] = u[i][(j * 3) + k] - (dt / (2.0 * dy)) * (fluxR_y[k] - fluxL_y[k]);
        }
      }
    }
    ...
    for (int j = 0; j <= ny + 3; j++) {
      for (int k = 0; k < 3; k++) {
        ...
        un[0][(j * 3) + k] = un[2][(j * 3) + k];
        un[1][(j * 3) + k] = un[2][(j * 3) + k];
        un[nx + 2][(j * 3) + k] = un[nx + 1][(j * 3) + k];
        un[nx + 3][(j * 3) + k] = un[nx + 1][(j * 3) + k];
      }
    }
    ...
    for (int i = 0; i <= nx + 3; i++) {
      for (int k = 0; k < 3; k++) {
        ...
        un[i][(0 * 3) + k] = un[i][(2 * 3) + k];
        un[i][(1 * 3) + k] = un[i][(2 * 3) + k];
        un[i][((ny + 2) * 3) + k] = un[i][((ny + 1) * 3) + k];
        un[i][((ny + 3) * 3) + k] = un[i][((ny + 1) * 3) + k];
      }
    }
      ...
      for (int i = 2; i <= nx + 1; i++) {
        for (int j = 2; j <= ny + 1; j++) {
          ...
          for (int k = 0; k < 3; k++) {
            ...
            output_data[k][(n * nx * ny) + ((i - 2) * ny) + (j - 2)][0] = u[i][(j * 3) + k];
          }
        }
      }
    }
...
;; -------------------------------------------------------------------------------------
;; Train a Roe (Finite-Volume) Surrogate Solver for a 2D Coupled Vector System of 3 PDEs
;; -------------------------------------------------------------------------------------
(define (train-roe-vector3-2d pde-system neural-net
                              #:nx [nx 200]
                              #:ny [ny 200]
                              #:x0 [x0 0.0]
                              #:x1 [x1 2.0]
                              ...
                              #:init-funcs [init-funcs (list
                                                        ...
                                                        `0.0
                                                        `0.0)])
 "Generate C code that trains a surrogate solver for the the 2D coupled vector system of 3 PDEs specified by `pde-system` using the Roe finite-volume method,
  with neural network architecture `neural-net`.
  - `nx`, `ny` : Number of spatial cells in each coordinate direction.
  - `x0`, `x1`, `y0`, `y1` : Domain boundaries in each coordinate direction.
  - `t-final`: Final time.
  ...
  (define code
    (format "
// AUTO-GENERATED CODE FOR TRAINING ON COUPLED VECTOR PDE SYSTEM: ~a
// Train a Roe higher-order finite-volume surrogate solver for a coupled vector system of 3 PDEs in 2D.
...
;; ----------------------------------------------------------------------------------------------------------------------------
;; Train a Roe (Finite-Volume) Surrogate Solver for a 2D Coupled Vector System of 3 PDEs with a Second-Order Flux Extrapolation
;; ----------------------------------------------------------------------------------------------------------------------------
(define (train-roe-vector3-2d-second-order pde-system limiter neural-net
                                           #:nx [nx 200]
                                           #:ny [ny 200]
                                           #:x0 [x0 0.0]
                                           #:x1 [x1 2.0]
                                           ...
                                           #:init-funcs [init-funcs (list
                                                                     ...
                                                                     `0.0
                                                                     `0.0)])
 "Generate C code that trains a surrogate solver for the 2D coupled vector system of 3 PDEs specified by `pde-system` using the Roe finite-volume method
...
// AUTO-GENERATED CODE FOR TRAINING ON COUPLED VECTOR PDE SYSTEM: ~a
// FLUX LIMITER: ~a
// Train a Roe higher-order finite-volume surrogate solver for a coupled vector system of 3 PDEs in 2D, with a second-order flux extrapolation.
