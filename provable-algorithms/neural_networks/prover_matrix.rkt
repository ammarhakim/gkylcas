...
(provide symbolic-diff
         ...
         prove-vector3-1d-non-smooth
         prove-vector4-2d-smooth
         prove-vector4-2d-non-smooth)
...
;; -----------------------------------------------------------------------------------------------------------------
;; Prove Error Bounds on Smooth Solutions for an Arbitrary Surrogate Solver for a 1D Coupled Vector System of 3 PDEs
;; -----------------------------------------------------------------------------------------------------------------
(define (prove-vector3-1d-smooth pde-system neural-net
                                 #:nx [nx 200]
                                 #:x0 [x0 0.0]
                                 #:x1 [x1 2.0]
                                 #:t-final [t-final 1.0]
                                 ...
                                 #:init-funcs [init-funcs (list
                                                           ...
                                                           `(cond
                                                              [(< x 0.5) 7.5]
                                                              [else 2.5]))])
   "Attempt to prove an analytic error bound on smooth solutions for an arbitrary surrogate solver for the 1D coupled vector system of 3 PDEs specified by `pde-system`,
  ...
  out)
(trace prove-vector3-1d-smooth)

;; ---------------------------------------------------------------------------------------------------------------------
;; Prove Error Bounds on Non-Smooth Solutions for an Arbitrary Surrogate Solver for a 1D Coupled Vector System of 3 PDEs
;; ---------------------------------------------------------------------------------------------------------------------
(define (prove-vector3-1d-non-smooth pde-system neural-net
                                     #:nx [nx 200]
                                     #:x0 [x0 0.0]
                                     #:x1 [x1 2.0]
                                     #:t-final [t-final 1.0]
                                     ...
                                     #:init-funcs [init-funcs (list
                                                               ...
                                                               `(cond
                                                                  [(< x 0.5) 7.5]
                                                                  [else 2.5]))])
   "Attempt to prove an analytic error bound on non-smooth solutions for an arbitrary surrogate solver for the 1D coupled vector system of 3 PDEs specified by `pde-system`,
    with neural network architecture `neural-net`.
  - `nx` : Number of spatial cells.
  - `x0`, `x1` : Domain boundaries.
  - `t-final`: Final time.
  ...
  out)
(trace prove-vector3-1d-non-smooth)

;; -----------------------------------------------------------------------------------------------------------------
;; Prove Error Bounds on Smooth Solutions for an Arbitrary Surrogate Solver for a 2D Coupled Vector System of 4 PDEs
;; -----------------------------------------------------------------------------------------------------------------
(define (prove-vector4-2d-smooth pde-system neural-net
                                 #:nx [nx 200]
                                 #:ny [ny 200]
                                 #:x0 [x0 0.0]
                                 #:x1 [x1 2.0]
                                 ...
                                 #:init-funcs [init-funcs (list
                                                           ...
                                                           `0.0
                                                           `0.0)])
   "Attempt to prove an analytic error bound on smooth solutions for an arbitrary surrogate solver for the 2D coupled vector system of 4 PDEs specified by `pde-system`,
    with neural network architecture `neural-net`.
  ...
  (define out (cond
    ...
    [(or (not (is-real (list-ref init-funcs 0) cons-exprs parameters))
         (not (is-real (list-ref init-funcs 1) cons-exprs parameters))
         (not (is-real (list-ref init-funcs 2) cons-exprs parameters))
         (not (is-real (list-ref init-funcs 3) cons-exprs parameters))) #f]
    ...
    [else (list (max (cond
                ...
                (max (cond
                       [(not (equal? (symbolic-simp `(< ,depth (+ 3 ,(list-ref (list-ref flux-jacobian-order-x 0) 3)))) #f)) +inf.0]
                       [else (symbolic-simp `(/ 1.0 (expt (* ,width ,depth) (/ 1.0 (+ 3 ,(list-ref (list-ref flux-jacobian-order-x 0) 3))))))])
                     (cond
                       [(not (equal? (symbolic-simp `(< ,depth (+ 3 ,(list-ref (list-ref flux-jacobian-order-y 0) 3)))) #f)) +inf.0]
                       [else (symbolic-simp `(/ 1.0 (expt (* ,width ,depth) (/ 1.0 (+ 3 ,(list-ref (list-ref flux-jacobian-order-y 0) 3))))))]))
                (max (cond
                ...
                (max (cond
                       [(not (equal? (symbolic-simp `(< ,depth (+ 3 ,(list-ref (list-ref flux-jacobian-order-x 1) 3)))) #f)) +inf.0]
                       [else (symbolic-simp `(/ 1.0 (expt (* ,width ,depth) (/ 1.0 (+ 3 ,(list-ref (list-ref flux-jacobian-order-x 1) 3))))))])
                     (cond
                       [(not (equal? (symbolic-simp `(< ,depth (+ 3 ,(list-ref (list-ref flux-jacobian-order-y 1) 3)))) #f)) +inf.0]
                       [else (symbolic-simp `(/ 1.0 (expt (* ,width ,depth) (/ 1.0 (+ 3 ,(list-ref (list-ref flux-jacobian-order-y 1) 3))))))]))
                (max (cond
                ...
                (max (cond
                       [(not (equal? (symbolic-simp `(< ,depth (+ 3 ,(list-ref (list-ref flux-jacobian-order-x 2) 3)))) #f)) +inf.0]
                       [else (symbolic-simp `(/ 1.0 (expt (* ,width ,depth) (/ 1.0 (+ 3 ,(list-ref (list-ref flux-jacobian-order-x 2) 3))))))])
                     (cond
                       [(not (equal? (symbolic-simp `(< ,depth (+ 3 ,(list-ref (list-ref flux-jacobian-order-y 2) 3)))) #f)) +inf.0]
                       [else (symbolic-simp `(/ 1.0 (expt (* ,width ,depth) (/ 1.0 (+ 3 ,(list-ref (list-ref flux-jacobian-order-y 2) 3))))))]))
                (max (cond
                       [(not (equal? (symbolic-simp `(< ,depth (+ 3 ,(list-ref (list-ref flux-jacobian-order-x 3) 0)))) #f)) +inf.0]
                       [else (symbolic-simp `(/ 1.0 (expt (* ,width ,depth) (/ 1.0 (+ 3 ,(list-ref (list-ref flux-jacobian-order-x 3) 0))))))])
                     (cond
                       [(not (equal? (symbolic-simp `(< ,depth (+ 3 ,(list-ref (list-ref flux-jacobian-order-y 3) 0)))) #f)) +inf.0]
                       [else (symbolic-simp `(/ 1.0 (expt (* ,width ,depth) (/ 1.0 (+ 3 ,(list-ref (list-ref flux-jacobian-order-y 3) 0))))))]))
                (max (cond
                       [(not (equal? (symbolic-simp `(< ,depth (+ 3 ,(list-ref (list-ref flux-jacobian-order-x 3) 1)))) #f)) +inf.0]
                       [else (symbolic-simp `(/ 1.0 (expt (* ,width ,depth) (/ 1.0 (+ 3 ,(list-ref (list-ref flux-jacobian-order-x 3) 1))))))])
                     (cond
                       [(not (equal? (symbolic-simp `(< ,depth (+ 3 ,(list-ref (list-ref flux-jacobian-order-y 3) 1)))) #f)) +inf.0]
                       [else (symbolic-simp `(/ 1.0 (expt (* ,width ,depth) (/ 1.0 (+ 3 ,(list-ref (list-ref flux-jacobian-order-y 3) 1))))))]))
                (max (cond
                       [(not (equal? (symbolic-simp `(< ,depth (+ 3 ,(list-ref (list-ref flux-jacobian-order-x 3) 2)))) #f)) +inf.0]
                       [else (symbolic-simp `(/ 1.0 (expt (* ,width ,depth) (/ 1.0 (+ 3 ,(list-ref (list-ref flux-jacobian-order-x 3) 2))))))])
                     (cond
                       [(not (equal? (symbolic-simp `(< ,depth (+ 3 ,(list-ref (list-ref flux-jacobian-order-y 3) 2)))) #f)) +inf.0]
                       [else (symbolic-simp `(/ 1.0 (expt (* ,width ,depth) (/ 1.0 (+ 3 ,(list-ref (list-ref flux-jacobian-order-y 3) 2))))))]))
                (max (cond
                       [(not (equal? (symbolic-simp `(< ,depth (+ 3 ,(list-ref (list-ref flux-jacobian-order-x 3) 3)))) #f)) +inf.0]
                       [else (symbolic-simp `(/ 1.0 (expt (* ,width ,depth) (/ 1.0 (+ 3 ,(list-ref (list-ref flux-jacobian-order-x 3) 3))))))])
                     (cond
                       [(not (equal? (symbolic-simp `(< ,depth (+ 3 ,(list-ref (list-ref flux-jacobian-order-y 3) 3)))) #f)) +inf.0]
                       [else (symbolic-simp `(/ 1.0 (expt (* ,width ,depth) (/ 1.0 (+ 3 ,(list-ref (list-ref flux-jacobian-order-y 3) 3))))))])))]))
  ...
  out)
(trace prove-vector4-2d-smooth)

;; -----------------------------------------------------------------------------------------------------------------
;; Prove Error Bounds on Non-Smooth Solutions for an Arbitrary Surrogate Solver for a 2D Coupled Vector System of 4 PDEs
;; -----------------------------------------------------------------------------------------------------------------
(define (prove-vector4-2d-non-smooth pde-system neural-net
                                     #:nx [nx 200]
                                     #:ny [ny 200]
                                     #:x0 [x0 0.0]
                                     #:x1 [x1 2.0]
                                     ...
                                     #:init-funcs [init-funcs (list
                                                               ...
                                                               `0.0
                                                               `0.0)])
   "Attempt to prove an analytic error bound on non-smooth solutions for an arbitrary surrogate solver for the 2D coupled vector system of 4 PDEs specified by `pde-system`,
    with neural network architecture `neural-net`.
  - `nx`, `ny` : Number of spatial cells in each coordinate direction.
  - `x0`, `x1`, `y0`, `y1` : Domain boundaries in each coordinate direction.
  - `t-final`: Final time.
  ...
  out)
(trace prove-vector4-2d-non-smooth)