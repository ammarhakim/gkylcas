...
(define (convert-expr expr)
  (match expr
    ...
    [`(equal? ,arg1 ,arg2)
     (format "fabs(~a - ~a) == 0.0" (convert-expr arg1) (convert-expr arg2))]

    ;; If expr is a negation of the form (not expr1), then convert it to "!(expr1)" in C.
    [`(not ,arg)
     (format "!(~a)" (convert-expr arg))]
