...
(provide symbolic-jacobian
         ...
         prove-roe-vector3-2d-strict-hyperbolicity-conditional
         prove-roe-vector3-2d-flux-conservation-conditional)
...
;; Recursively determine whether an expression corresponds to a real number subject to certain algebraic conditions.
(define (is-real-conditional expr cons-vars parameters conds)
  (match expr
    ...
    [`(cond
        [,cond1 ,expr1]
        [else ,expr2])
     (and (is-real-conditional expr1 cons-vars parameters conds) (is-real-conditional expr2 cons-vars parameters conds))]
    ...
    [`(expt ,arg)
     (is-real-conditional arg cons-vars parameters conds)]
    ...
    [`(+ . ,terms)
     (andmap (lambda (term) (is-real-conditional term cons-vars parameters conds)) terms)]
    [`(- . ,terms)
     (andmap (lambda (term) (is-real-conditional term cons-vars parameters conds)) terms)]
    [`(* . ,terms)
     (andmap (lambda (term) (is-real-conditional term cons-vars parameters conds)) terms)]
...
;; ---------------------------------------------------------------------------------------------------------------------------------------------------------
(define (prove-lax-friedrichs-vector2-1d-local-lipschitz-conditional pde-system conds
                                                                     #:nx [nx 200]
                                                                     #:x0 [x0 0.0]
                                                                     #:x1 [x1 2.0]
                                                                     #:t-final [t-final 1.0]
  ...
  (define out (cond
    ...
    ;; Check whether the simulation parameter(s) correspond to real numbers (otherwise, return false).r
    [(not (or (empty? parameters) (andmap (lambda (parameter)
                                            (is-real-conditional (list-ref parameter 2) cons-exprs parameters conds)) parameters))) #f]
...
;; ---------------------------------------------------------------------------------------------------------------------------------------------
;; Prove strict hyperbolicity of the Roe (Finite-Volume) Solver for a 2D Coupled Vector System of 3 PDEs subject to certain algebraic conditions
;; ---------------------------------------------------------------------------------------------------------------------------------------------
(define (prove-roe-vector3-2d-strict-hyperbolicity-conditional pde-system conds
                                                               #:nx [nx 200]
                                                               #:ny [ny 200]
                                                               #:x0 [x0 0.0]
                                                               #:x1 [x1 2.0]
  ...
  (define out (cond
    ...
    [(not (are-distinct-conditional (list (list-ref roe-matrix-eigvals-simp-x 0) (list-ref roe-matrix-eigvals-simp-x 1)) parameters conds)) #f]
    [(not (are-distinct-conditional (list (list-ref roe-matrix-eigvals-simp-x 0) (list-ref roe-matrix-eigvals-simp-x 2)) parameters conds)) #f]
    [(not (are-distinct-conditional (list (list-ref roe-matrix-eigvals-simp-x 1) (list-ref roe-matrix-eigvals-simp-x 2)) parameters conds)) #f]
    [(not (are-distinct-conditional (list (list-ref roe-matrix-eigvals-simp-y 0) (list-ref roe-matrix-eigvals-simp-y 1)) parameters conds)) #f]
    [(not (are-distinct-conditional (list (list-ref roe-matrix-eigvals-simp-y 0) (list-ref roe-matrix-eigvals-simp-y 2)) parameters conds)) #f]
    [(not (are-distinct-conditional (list (list-ref roe-matrix-eigvals-simp-y 1) (list-ref roe-matrix-eigvals-simp-y 2)) parameters conds)) #f]
  ...
  out)
(trace prove-roe-vector3-2d-strict-hyperbolicity-conditional)

;; ------------------------------------------------------------------------------------------------------------------------------------------------------------
;; Prove flux conservation (jump continuity) of the Roe (Finite-Volume) Solver for a 2D Coupled Vector System of 3 PDEs subject to certain algebraic conditions
;; ------------------------------------------------------------------------------------------------------------------------------------------------------------
(define (prove-roe-vector3-2d-flux-conservation-conditional pde-system conds
                                                            #:nx [nx 200]
                                                            #:ny [ny 200]
                                                            #:x0 [x0 0.0]
                                                            #:x1 [x1 2.0]
  ...
  (define out (cond
    ...
    [(or (not (are-equal-conditional (list (list-ref roe-jump-x 0) (list-ref flux-jump-x 0)) parameters conds))
         (not (are-equal-conditional (list (list-ref roe-jump-x 1) (list-ref flux-jump-x 1)) parameters conds))
         (not (are-equal-conditional (list (list-ref roe-jump-x 2) (list-ref flux-jump-x 2)) parameters conds))) #f]
    [(or (not (are-equal-conditional (list (list-ref roe-jump-y 0) (list-ref flux-jump-y 0)) parameters conds))
         (not (are-equal-conditional (list (list-ref roe-jump-y 1) (list-ref flux-jump-y 1)) parameters conds))
         (not (are-equal-conditional (list (list-ref roe-jump-y 2) (list-ref flux-jump-y 2)) parameters conds))) #f]
  ...
  out)
(trace prove-roe-vector3-2d-flux-conservation-conditional)