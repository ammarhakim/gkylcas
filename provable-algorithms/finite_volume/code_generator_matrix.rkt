...
(provide generate-lax-friedrichs-vector3-1d
         generate-lax-friedrichs-vector3-1d-second-order
         generate-roe-vector3-1d
         generate-roe-vector3-1d-second-order)
...
int main() {
  ...
  for (int i = 0; i <= nx + 1; i++) {
    ...
    un[(i * 3) + 0] = ~a; // init-funcs[0] in C.
    un[(i * 3) + 1] = ~a; // init-funcs[1] in C.
    un[(i * 3) + 2] = ~a; // init-funcs[2] in C.
  }
...
;; -------------------------------------------------------------------------------------------------------------------------
;; Lax–Friedrichs (Finite-Difference) Solver for a 1D Coupled Vector System of 3 PDEs with a Second-Order Flux Extrapolation
;; -------------------------------------------------------------------------------------------------------------------------
(define (generate-lax-friedrichs-vector3-1d-second-order pde-system limiter
                                                         #:nx [nx 200]
                                                         #:x0 [x0 0.0]
                                                         #:x1 [x1 2.0]
                                                         #:t-final [t-final 1.0]
                                                         ...
                                                         #:init-funcs [init-funcs (list
                                                                                   ...
                                                                                   `(cond
                                                                                      [(< x 0.5) 7.5]
                                                                                      [else 2.5]))])
  "Generate C code that solves for the 1D coupled vector system of 3 PDEs specified by `pde-system` using the Lax-Friedrichs finite-difference method
   with a second-order flux extrapolation using the limiter `limiter`.
  - `nx` : Number of spatial cells.
  - `x0`, `x1` : Domain boundaries.
  - `t-final`: Final time.
  - `cfl`: CFL coefficient.
...
// FLUX LIMITER: ~a
// Lax–Friedrichs first-order finite-difference solver for a coupled vector system of 3 PDEs in 1D, with a second-order flux extrapolation.
...
;; -------------------------------------------------------------------
;; Roe (Finite-Volume) Solver for a 1D Coupled Vector System of 3 PDEs
;; -------------------------------------------------------------------
(define (generate-roe-vector3-1d pde-system
                                 #:nx [nx 200]
                                 #:x0 [x0 0.0]
                                 #:x1 [x1 2.0]
                                 #:t-final [t-final 1.0]
                                 ...
                                 #:init-funcs [init-funcs (list
                                                           ...
                                                           `(cond
                                                              [(< x 0.5) 7.5]
                                                              [else 2.5]))])
 "Generate C code that solves the 1D coupled vector system of 3 PDEs specified by `pde-system` using the Roe finite-volume method.
  - `nx` : Number of spatial cells.
  - `x0`, `x1` : Domain boundaries.
  - `t-final`: Final time.
  - `cfl`: CFL coefficient.
  ...
  (define flux-jacobian-eigvals (symbolic-eigvals3 (symbolic-jacobian flux-exprs cons-exprs)))
  (define flux-jacobian-eigvals-simp (list (symbolic-simp (list-ref flux-jacobian-eigvals 0))
                                           ...
                                           (symbolic-simp (list-ref flux-jacobian-eigvals 2))))
  ...
  (define flux-deriv-ums (map (lambda (flux-deriv-code)
                                (flux-substitute (flux-substitute (flux-substitute flux-deriv-code (list-ref cons-codes 0) "um[0]")
                                                                  (list-ref cons-codes 1) "um[1]") (list-ref cons-codes 2) "um[2]")) flux-deriv-codes))
  (define flux-deriv-uis (map (lambda (flux-deriv-code)
                                (flux-substitute (flux-substitute (flux-substitute flux-deriv-code (list-ref cons-codes 0) "ui[0]")
                                                                  (list-ref cons-codes 1) "ui[1]") (list-ref cons-codes 2) "ui[2]")) flux-deriv-codes))
  (define flux-deriv-ups (map (lambda (flux-deriv-code)
                                (flux-substitute (flux-substitute (flux-substitute flux-deriv-code (list-ref cons-codes 0) "up[0]")
                                                                  (list-ref cons-codes 1) "up[1]") (list-ref cons-codes 2) "up[2]")) flux-deriv-codes))
  ...
  (define code
    (format "
// AUTO-GENERATED CODE FOR COUPLED VECTOR PDE SYSTEM: ~a
// Roe higher-order finite-volume solver for a coupled vector system of 3 PDEs in 1D.
...
int main() {
  ...
  double *f_deriv_um = (double*) malloc(3 * sizeof(double));
  double *f_deriv_ui = (double*) malloc(3 * sizeof(double));
  double *f_deriv_up = (double*) malloc(3 * sizeof(double));

  double *aL_roe = (double*) malloc(3 * sizeof(double));
  double *aR_roe = (double*) malloc(3 * sizeof(double));
  ...
  while (t < t_final) {
    ...
    for (int i = 1; i <= nx; i++) {
      ...
      f_deriv_um[0] = ~a;
      f_deriv_um[1] = ~a;
      f_deriv_um[2] = ~a;// Eigenvalues of F'(U_{i - 1}).
      
      f_deriv_ui[0] = ~a;
      f_deriv_ui[1] = ~a;
      f_deriv_ui[2] = ~a;// Eigenvalues of F'(U_i).
      
      f_deriv_up[0] = ~a;
      f_deriv_up[1] = ~a;
      f_deriv_up[2] = ~a;// Eigenvalues of F'(U_{i + 1}).
...
;; ----------------------------------------------------------------------------------------------------------
;; Roe (Finite-Volume) Solver for a 1D Coupled Vector System of 3 PDEs with a Second-Order Flux Extrapolation
;; ----------------------------------------------------------------------------------------------------------
(define (generate-roe-vector3-1d-second-order pde-system limiter
                                              #:nx [nx 200]
                                              #:x0 [x0 0.0]
                                              #:x1 [x1 2.0]
                                              #:t-final [t-final 1.0]
                                              ...
                                              #:init-funcs [init-funcs (list
                                                                        ...
                                                                        `(cond
                                                                           [(< x 0.5) 7.5]
                                                                           [else 2.5]))])
 "Generate C code that solves the 1D coupled vector system of 3 PDEs specified by `pde-system` using the Roe finite-volume method with a
  ...
  (define flux-deriv-umR-evols (map (lambda (flux-deriv-code)
                                      (flux-substitute (flux-substitute (flux-substitute flux-deriv-code (list-ref cons-codes 0) "umR_evol[0]")
                                                                        (list-ref cons-codes 1) "umR_evol[1]") (list-ref cons-codes 2) "umR_evol[2]")) flux-deriv-codes))
  (define flux-deriv-uiL-evols (map (lambda (flux-deriv-code)
                                      (flux-substitute (flux-substitute (flux-substitute flux-deriv-code (list-ref cons-codes 0) "uiL_evol[0]")
                                                                        (list-ref cons-codes 1) "uiL_evol[1]") (list-ref cons-codes 2) "uiL_evol[2]")) flux-deriv-codes))
  (define flux-deriv-uiR-evols (map (lambda (flux-deriv-code)
                                      (flux-substitute (flux-substitute (flux-substitute flux-deriv-code (list-ref cons-codes 0) "uiR_evol[0]")
                                                                        (list-ref cons-codes 1) "uiR_evol[1]") (list-ref cons-codes 2) "uiR_evol[2]")) flux-deriv-codes))
  (define flux-deriv-upL-evols (map (lambda (flux-deriv-code)
                                      (flux-substitute (flux-substitute (flux-substitute flux-deriv-code (list-ref cons-codes 0) "upL_evol[0]")
                                                                        (list-ref cons-codes 1) "upL_evol[1]") (list-ref cons-codes 2) "upL_evol[2]")) flux-deriv-codes))
...
// AUTO-GENERATED CODE FOR COUPLED VECTOR PDE SYSTEM: ~a
// FLUX LIMITER: ~a
// Roe higher-order finite-volume solver for a coupled vector system of 3 PDEs in 1D, with a second-order flux extrapolation.
...
int main() {
  ...
  double *f_deriv_umR_evol = (double*) malloc(3 * sizeof(double));
  double *f_deriv_uiL_evol = (double*) malloc(3 * sizeof(double));
  double *f_deriv_uiR_evol = (double*) malloc(3 * sizeof(double));
  double *f_deriv_upL_evol = (double*) malloc(3 * sizeof(double));
  ...
  while (t < t_final) {
    ...
    for (int i = 2; i <= nx + 1; i++) {
      ...
      f_deriv_umR_evol[1] = ~a;
      f_deriv_umR_evol[2] = ~a; // F'(U_{i - 1, R+})
      f_deriv_uiL_evol[0] = ~a;
      f_deriv_uiL_evol[1] = ~a;
      f_deriv_uiL_evol[2] = ~a; // F'(U_{i, L+})
      
      f_deriv_uiR_evol[0] = ~a;
      f_deriv_uiR_evol[1] = ~a;
      f_deriv_uiR_evol[2] = ~a; // F'(U_{i, R+})
      f_deriv_upL_evol[0] = ~a;
      f_deriv_upL_evol[1] = ~a;
      f_deriv_upL_evol[2] = ~a; // F'(U_{i + 1, L+})
...
"
           ...
           (list-ref flux-deriv-uiR-evols 2)
           ...
           (list-ref flux-deriv-upL-evols 2)
           ;; PDE name for file output.
           name
           name
           ))
