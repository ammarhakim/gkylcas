...
(provide generate-lax-friedrichs-vector3-1d-conditional
         generate-lax-friedrichs-vector3-1d-second-order-conditional)
...
;; ------------------------------------------------------------------------------------------------------------------------------------------------------------------
;; Lax–Friedrichs (Finite-Difference) Solver for a 1D Coupled Vector System of 3 PDEs with a Second-Order Flux Extrapolation subject to certain algebraic constraints
;; ------------------------------------------------------------------------------------------------------------------------------------------------------------------
(define (generate-lax-friedrichs-vector3-1d-second-order-conditional pde-system limiter conds epsilon
                                                                     #:nx [nx 200]
                                                                     #:x0 [x0 0.0]
                                                                     #:x1 [x1 2.0]
                                                                     #:t-final [t-final 1.0]
  ...
  "Generate C code that solves for the 1D coupled vector system of 3 PDEs specified by `pde-system` using the Lax-Friedrichs finite-difference method
   with a second-order flux extrapolation using the limiter `limiter`, subject to the algebraic conditions `conds` with machine epsilon `epsilon`.
  - `nx` : Number of spatial cells.
  - `x0`, `x1` : Domain boundaries.
  - `t-final`: Final time.
  - `cfl`: CFL coefficient.
...
// FLUX LIMITER: ~a
// Lax–Friedrichs first-order finite-difference solver for a coupled vector system of 3 PDEs in 1D, with a second-order flux extrapolation subject to algebraic constraints.
